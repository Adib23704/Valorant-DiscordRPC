...
impl Lockfile {
  pub fn default_path() -> AppResult<PathBuf> {
    let local_app_data = std::env::var("LOCALAPPDATA")
      .map_err(|_| AppError::Lockfile("LOCALAPPDATA environment variable not found".into()))?;
  ...
  pub fn read_from_path(path: impl AsRef<Path>) -> AppResult<Self> {
  ...
  fn parse(content: &str) -> AppResult<Self> {
    ...
    let process_id = parts[1]
      .parse::<u32>()
      .map_err(|_| AppError::Lockfile(format!("Invalid process ID in lockfile: {}", parts[1])))?;
